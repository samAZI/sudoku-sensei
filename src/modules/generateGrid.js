...
const generate = () => {
    return grid.map((row, y) => {
        return row.map((column, x) => {
            ...
            let number
            let iterator = 0
            let debugNumbers = {row: 0, column: 0, square: 0}
            let limitTries = 1000

            while (!isValid && (iterator < limitTries) ) {
                number = getRandomNumber(1, 9)
                let isValidForColumn = isValidNumberForColumn(number, x)
                let isValidForRow = isValidNumberForRow(number, y)
                let isValidForSquare = isValidNumberForSquare(number, x, y)
                isValid = isValidForSquare && isValidForRow && isValidForColumn

                if (isValidForSquare === false) debugNumbers.square += 1
                if (isValidForRow === false) debugNumbers.row += 1
                if (isValidForColumn === false) debugNumbers.column += 1

                iterator++

                if (iterator === limitTries) {
                    console.log('x:', x,'y:', y,'iterator:', iterator,'number:', number)
                    console.log(debugNumbers)
                }
...
function isValidNumberForRow(number, y) {
    for (let iterator = 0; iterator < 9; iterator++) {
        // console.log('      number:', number, 'y:', y, 'grid['+y+']['+iterator+']', grid[y][iterator])
        if (grid[y][iterator] === number) {
            // console.log('random === grid point')
            return false
        }
    }
    return true
...
function isValidNumberForColumn(number, x) {
    for (let iterator = 0; iterator < 9; iterator++) {
        // console.log('      number:', number, 'x:', x, 'grid['+iterator+']['+x+']', grid[iterator][x])
        if (grid[iterator][x] === number) {
            // console.log('random === grid point')
            return false
        }
    }
